...
- Measurement-only — no auto-correction shim. Rationale: measurement noise floor (onset-detection jitter accumulated over 60s) is ~5-10 ms/min, so any correction below that would chase noise; iOS already does audio-clock-vs-system-clock reconciliation internally; and if drift ever exceeds spec, that's a code bug worth fixing at the source, not papering over per-device.
- v0.34.1 fixed two real-device bugs caught the first time anyone ran the test: (1) the diagnostic was using `scheduler.format.sampleRate` cached at app-init time (typically 44.1 kHz before audio session activated) while iOS actually rendered at 48 kHz once the session went live, producing a constant 1.088× ratio that made periods appear 8.8% longer than they were — fix reads `buffer.format.sampleRate` per callback; (2) engine state wasn't snapshotted / restored, so any active polyrhythm / voice-count / random-mute setting polluted the captured audio — fix snapshots `EngineSettings` + `currentSoundPreset` + `currentPolyrhythmOverride` before the test and restores them after, even on early exit. v0.34.1 also adds a Swift Charts IOI plot to the result so users can see per-click distribution rather than relying on the median number alone.
