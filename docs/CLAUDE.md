...
- **Testing strategy**: unit-test against `FakeClock` in `Tests/MetronomeCoreTests/`. No audio output is exercised in tests; drift verification at the math level only. The full audio path drift test (spec §1.1) ships as an in-app diagnostic at Settings → Diagnostics → Drift Self-Test (`DriftSelfTest.swift`) — taps `mainMixerNode`, detects onsets via RMS energy + refractory, reports median-IOI drift vs the 1 ms/min spec budget. Measurement-only, no auto-correction.
...
- **Tap tempo**: window of **last 4 taps**, reset after **2 s** of inactivity. BPM = **median** of inter-tap intervals (not arithmetic mean — robust to a single outlier tap; v0.34.2). Taps closer than **100 ms** to the previous one are rejected as gesture-system double-fires.
...
- **Phase 3 (pro) — partial:** §12.2 MIDI Clock send + receive ✓ (Settings → MIDI source picker v0.18.0; Song Position Pointer v0.19.0); §7.3 multi-section songs ✓ (per-section state, D.C. al Fine, repeats, drag-to-reorder, full setlist integration across all 3 advance modes incl. `.countdown` count-in into multi-section songs, per-section custom time-signature picker v0.24.0 — D.S./D.C. al Coda jumps shipped v0.27.0); §2.4 polyrhythm ✓ (same-measure flavor shipped v0.30.0 — engine math, audio rendering through a second AVAudioPlayerNode, Settings + per-song override, Stage hollow-dot indicator); §12.1 BLE pedals, §12.3 Ableton Link, §13 Apple Watch, §14 iCloud — **user has explicitly dropped these** (TODO.md).
- **Phase 4 (polish):** §15 accessibility audit closed — VoiceOver / Dynamic Type / Reduce Motion / sibling-label slider work landed v0.27–v0.30.2; high-contrast and Switch Control compatibility real-device verified v0.32.6. User-imported sounds (§4.2) shipped v0.31.0 with full import-validate-cache-play loop. iPad polish (GeometryReader + collapsible dock) v0.32.0. Volume-key start/stop + headphone-remote audit v0.32.1, real-device verified v0.32.5–v0.32.6. Now Playing (§16) end-to-end verified v0.32.5 — the critical wire-up was finding that `.mixWithOthers` was permanently set and never reached the audio session coordinator (now a Settings toggle, default OFF). Haptic defaults (§9) verified v0.32.6 — feel right as shipped, no tuning required. Real percussion samples (§4.1) and bundled voice count samples (§5) dropped indefinitely — user prefers no pre-recorded audio baked into the app binary.
