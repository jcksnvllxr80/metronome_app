...
- ~~v0.14.4 (current)~~: new `AudioScheduler.scheduleResetWithCap(_:)` updates the cap AND refills in a single actor-isolated call. SongSectionPlayer calls this after `engine.apply`, after the apply chain's reset Tasks have run (and done nothing useful). The combined call updates the cap to the NEW section's boundary first, then queues NEW section's clicks — they land correctly at the new tempo. Verify on device.
