...
### ~~Double-click at section boundaries (old + new first click)~~ — attempted fixes in v0.14.2, v0.14.3
v0.14.2 tried adding a `scheduleResetWithFlush` (playerNode.stop()+play()) on transition — still produced the double click on device. Root cause: by the time the section transition fires + the apply chain + the await context-switching runs, the boundary hostTime has already passed and the OLD section's queued boundary click has already played. No amount of late flushing can reach into the past.

v0.14.3 takes a different angle: prevent the OLD section's boundary click from ever entering the queue. The `AudioScheduler` gains a `schedulingEndTime` cap that `SongSectionPlayer` updates on every play/advance/repeat/D.C. jump to the current section's natural boundary time. The refill loop strict-less-than-checks `click.time < endTime` before scheduling — so the click that conceptually belongs to the NEXT section's measure 0 (at the boundary hostTime) never makes it into the player's queue. The new section's first click is the only one scheduled at the boundary hostTime, no double. Verify on device.
