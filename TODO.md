...
### ~~Double-click at section boundaries (old + new first click)~~ — fixed in v0.14.2
v0.14.1 timed the section transition correctly, but device QA then reported hearing two clicks at the boundary: the would-be next-measure downbeat of the OLD section plus the first click of the NEW section. Root cause: by the time the section transition fires, the audio scheduler has already queued the OLD section's click that would have been the next-measure downbeat (at the boundary hostTime, in the player's queue). The engine's standard reanchor path uses `AVAudioPlayerNodeBufferOptions.interrupts` on the first new buffer — but `.interrupts` only preempts the buffer currently rendering, not future-queued buffers. So the OLD section's queued boundary click still played at its scheduled time, alongside the NEW section's first click. Fix: section transitions now call a new `AudioScheduler.scheduleResetWithFlush()` that does `playerNode.stop()` + `play()` to actually drop the queued buffers before re-queueing. Tempo nudges keep the soft `.interrupts` path since they don't have queued-past-boundary clicks. Same flush logic on D.C. al Fine jumps and within-section repeat passes.
