...
- `.pause` advance mode + multi-section: when a setlist with `.pause` advance hits a multi-section song, after the prior song ends the engine pauses on the multi-section song's top-level BPM (not section 0's). On user-press-Play, playback resumes at the top-level BPM with no section transitions — currently treats the multi-section song as flat for the .pause-resume path. Fix would route togglePlay through the section player when resuming a setlist-paused multi-section song.
- Count-in (`.countdown` advance mode) on multi-section songs — the section-player routing skips the count-in measures (those flow through `engine.start(countIn:)` only on the flat path). Could be plumbed through by extending `SongSectionPlayer.play` to accept a count-in parameter forwarded to `engine.start`.
