...
- **Phase 2 (practice tools) — mostly shipped:** §5 voice count *scaffold + .beats mode* ✓, full language/gender sample matrix ✗; §7.1–7.2 songs/setlists ✓ (incl. accent pattern editor, setlist auto-advance); §6.3 tempo automation — gradual ramp + step BPM ✓ (ramp loops ✗); §6.4 speed trainer — random mute + step BPM ✓ (stop/reverse-on-ceiling + "successful loops" trigger ✗); §11 practice stats ✓ (session log, today/week/month totals, per-song breakdown, CSV export); §9 haptics ✗.
