...
- **Phase 4 (polish):** §15 accessibility audit, latency tuning, edge cases — partially in place (Reduce Motion respected across BPM-digit fade + beat-dot color animations as of v0.30.1; Dynamic Type via DS.Font semantic anchors v0.30.1; sibling-label slider VoiceOver fix v0.30.2; VoiceOver pass on Stage + Library surfaces from v0.27–0.28 covers play/stop + BPM announcements, selected-state traits, beat-sequence labels). User-imported sounds (§4.2) shipped v0.31.0 with the full import-validate-cache-play loop. Real percussion samples (§4.1) and bundled voice count samples (§5) dropped indefinitely — user prefers no pre-recorded audio baked into the app binary.
