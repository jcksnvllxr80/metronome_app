...
- **Phase 4 (polish):** §15 accessibility audit, latency tuning, edge cases — partially in place (Reduce Motion respected; VoiceOver pass on Stage + Library surfaces in v0.27–0.28 covering play/stop + BPM announcements, selected-state traits on picker rows, AccentPatternLibrary beat-sequence labels). User-imported sounds (§4.2) is the remaining sound-library work — real percussion samples (§4.1) and bundled voice count samples (§5) were dropped indefinitely since they'd bake audio into the app binary.
